...
import binder.BindFieldNonNull;
import binder.BindPropNonNull;
...
public class BinderTests {
	...
	public void testBindTo() {
		...
		try {
			testObjectDTO = new Binder(new BindFieldNonNull()).bindTo(TestObjectDTO.class, Binder.getFieldsValues(testObject));
	...
	public void testBindToProperties() {
		...
		TestObjectDTO testObjectDTO = null;
		try {
			testObjectDTO = new Binder(new BindPropNonNull()).bindTo(TestObjectDTO.class, Binder.getFieldsValues(testObject));
			assertTrue(testObjectDTO != null);
			assertTrue(testObjectDTO.getClass().getField("testField1") != null );
	...
	public void testBindToFieldsAndProperties() {
		...
		TestObjectDTO testObjectDTO = null;
		try {
			testObjectDTO = new Binder(new BindFieldNonNull(), new BindPropNonNull()).bindTo(TestObjectDTO.class, Binder.getFieldsValues(testObject));
			assertTrue(testObjectDTO != null);
			assertTrue(testObjectDTO.getClass().getField("testField1") != null );
			for (Field f : testObjectDTO.getClass().getFields()){
				if(f.getType().getName().equals(
